...
## 49. sheetagent 在线化：C 端接口 / MCP 取票方案 / 产物同步 / 关键代码位置

### 49.1 C 端接口清单

| 接口 | 说明 |
|---|---|
| api/v6/recommend/ai/quick_entries | 生成PPT等一级分类 |
| api/v6/agentgateway/chat | 流式会话 |
| api/v6/agentgateway/runtimes | 会话记录 |
| api/v6/agentgateway/history | 单个会话历史 |
| api/v6/agentgateway/cancel | 取消会话 |
| api/v6/agentgateway/runtime/artifacts | 会话制品 |
| api/v6/agentgateway/runtime/pin | 会话置顶 |
| api/v6/agentgateway/runtime/rename | 会话重命名 |
| api/v6/agentgateway/runtime/delete | 会话删除 |

### 49.2 MCP 取票通道：方案 A vs 方案 B

MCP 场景里，满足「运行时 + 保命可得 + 无时序」的通道是现成的：`CODEBUDDY_SERVICE_PROXY_URL`（daemon 级、spawn 时就有、不依赖 session 初始化，宿主在 `instantiateServer` 里无条件注入）。所以两条路：

| | 方案 A：运行时走 service proxy 取票 | 方案 B：spawn 时注入 connector-proxy config |
|---|---|---|
| 做法 | workbuddy-server 在 loopback 加一个「取票」internal 端点；MCP 首次出站启用 `CODEBUDDY_SERVICE_PROXY_URL` 调它取票，取票逻辑留在宿主侧（复用连接器授权上下文） | `instantiateServer` 里 `getByName('connector-proxy')`，查到就注入 `CODEBUDDY_MCP_CONFIG`，查不到跳过 |
| 时序 | 零时序问题（通道是 daemon 级） | 有——需查证 connector-proxy 在 DYNAMIC scope，eager spawn 早于其初始化 |
| 实测风险 | 需给取票端点加个鉴权（loopback + 短命 secret） | `getByName` 有 10s 超时兜底（不会死锁），但 eager 时查不到 → 注入失败，且 PLUGIN scope 的 server 未必被 DYNAMIC 变化触发 rebuild → 可能一直没凭据，得重开对话 |
| 改动量 | 宿主 ~50 行 + MCP `credentials.ts` 改通道 | 宿主 ~30 行 |

**建议选 A**。理由：它是「参考之前方案」的真正等价物——之前是「运行时从 Bash env 取票」，A 是「运行时从 service proxy 取票」，同构、无时序。B 只是把时序问题用「短超时 + 碰运气 rebuild」压下去，没有真正消除；而且 `CODEBUDDY_MCP_CONFIG` 里还带着别的 connector 配置，注入给 MCP 进程有额外的暴露面。

### 49.3 三类插件的鉴权位置对比

| 东西 | 鉴权在哪 | 为什么 |
|---|---|---|
| sheetagent（改前） | 无 | 连本地 editorsdk（`127.0.0.1:39099/mcp`），编辑本地文件，不碰线上 |
| tencent-docs-plugin（改前） | Bash 沙箱里 | 它没有 MCP server，取票全靠 tencentdocs.py 这个 skill 脚本在 Bash 里跑。Bash 的 env 是宿主显式构造的（bash-context-env.ts），里面有 `CODEBUDDY_MCP_CONFIG` → 脚本靠它找 connector-proxy 取票 |
| 在线 sheetagent（要加的） | MCP server 进程里 | sheetagent 是 `node start.mjs` 独立进程，MCP SDK 只给它最小 env，没有 `CODEBUDDY_MCP_CONFIG` → 取不到票 |

所以「tencent-docs-plugin 没安全问题」不是因为它鉴权做得更好，而是因为它的鉴权根本不在 MCP 进程里，在 Bash 里。

### 49.4 build:workbuddy 产物手工同步到 genie

```bash
cd docx-online/docx-ai/src/sheet

# 1. 构建 WORKBUDDY 产物（含双 server + sheet-agent-online + 在线 prompt）
npm run build:workbuddy
# 产物：dist/plugins/sheetagent/（.codebuddy-plugin + commands + hooks + agents + prompt + skills + mcp）

# 2. 覆盖 genie builtin-plugins（genie 侧 sync 脚本不存在，只能手工）
rm -rf ~/Documents/git-woa/3.docx/genie/apps/workbuddy-desktop/resources/plugins/workbuddy-builtin/builtin-plugins/sheetagent
cp -r dist/plugins/sheetagent ~/Documents/git-woa/3.docx/genie/apps/workbuddy-desktop/resources/plugins/workbuddy-builtin/builtin-plugins/sheetagent
```

### 49.5 关键代码位置

```text
editor_sdk 位置           docx-online/library/collab/editor/
sheetengine 位置          docx-online/backend/application/collabengine/sheetengine
editor_sdk 工具定义位置    docx-online/library/collab/editor/server/mcp/sheet/（如 pivot_tools.cc）
```
