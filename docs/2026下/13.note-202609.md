...
### 3. 问题：`injector.get(UserService)` 做了什么？
...
### 1. `@contextmanager` 装饰器（SQLAlchemy 教程里用到的）
...
### 4. 底层：最常见的 B+ 树索引（MySQL InnoDB 默认）
...
### 31.1 函数视角：ChatGPT 是一个"接收输入、返回一个词"的函数
...
## 33. 从 One-Hot 到 Embedding

### 33.1 One-Hot 编码及其两大问题
...
## 34. 预训练：自监督与 Next-Token Prediction
...
### 34.5 术语：petaflop 与 Gemini / Gemma
...
### 36.3 In-Context Learning（ICL）

**ICL（上下文学习）**：在 Prompt 中放入类比示例，让模型照着示例的模式完成任务。

### 36.4 思维链 CoT：定义、两种形式、为什么有效
...
## 38. Weaviate 升级：hfresh 索引类型与 deprecated API
...
### worker线程的op为什么会给主线程执行，直接在worker线程里执行不行吗，worker线程的作用是啥

这个问题问到架构根子上了。**答案是：不行，会死锁**。我从代码里给你逐层拆。

#### 最本质的原因：`sendSync` 会把 worker 线程冻死
...
### interface.ts 给谁用的，protocol.ts 给谁用的

这两个文件是**同一条链上两个不同的闸门**，服务完全不同的对象。

#### 一、各自给谁用

##### `api/interface.ts` —— 能力契约（MCP 工具层用）
...
##### `runtime/protocol.ts` —— 通信协议（run_command 通道用）
...
## run_command 和其他工具的本质区别是什么，用哪个更优

这是个很关键的架构问题。我先把 shim 的能力面查清楚了——**结论可能和你预期不同：run_command 现在几乎什么都能做**。

#### 一、run_command 的真实能力边界 = shim 暴露了什么
