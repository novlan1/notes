...
> 本文档结合 `ai-e2e-platform` 中两条平行实验线（`tests/` 用 Midscene，`browser-use-lab/` 用 Browser-Use），从架构原理层面拆解二者的根本差异，帮助选型与排错。
...
> 由于 DOM 树同时被喂入，模型在"和平精英"这种纯文本目标上不需要纯视觉 OCR，准确率比纯截图方案高。这就是 `tests/gp-next-ai.spec.ts` 里 `aiQuery` 能稳定抽出卡片标题的根本原因。
...
4. **`initial_actions` 旁路**：像我们 `browser-use-lab/tasks/_shared.py` 里那样，**确定性的步骤（鉴权 URL → 业务 URL）走 `initial_actions`，不消耗 token、不进决策循环**——这是 Agent 系统典型的"绕过 LLM"优化。
...
## 五、本项目实测对照（基于 `gp-next-ai.spec.ts` vs `browser-use-lab/tasks/02_search_keyword.py`）
...
两边共用同一个 **Knot OpenAI 兼容代理**（`127.0.0.1:3939/v1`，见 `scripts/proxy-server.ts`），账单和模型切换都集中在一处。
