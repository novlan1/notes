...
<img src="https://cdn.uwayfly.com/article/2026/4/own_mike_yGmGzkXCfz7kjMSJ.png" width="800" />
...
```mermaid
graph TD
    A[MR 创建] --> B[AI 评审: 发现 N 个 issues]
    B --> C[提交 N 条行内评论]
    C --> D[用户在某条行内评论下回复 /ai-fix]
    D --> E[提取上下文: file, line, parentNote]
    E --> F[通过评论内容匹配: N→1 issue]
    F --> G[AI 修复: 只修复匹配的 issue]
    G --> H[提交到原分支]
    H --> I[跳过重新评审 ✅]
```
...
**方案演进**：
1. 最初尝试使用 `in_reply_to_id` 参数，但工蜂对 Review 类型的行内评审讨论串不生效
2. 改用工蜂 **discussions API** 回复到讨论串中
...
sequenceDiagram
    ...
    participant Git as 工蜂 API

    User->>Git: 在行内评审讨论串中回复 /ai-fix
    Git->>WH: Note Webhook (noteAttrs.id=123)
    WH->>AF: handleNoteWebhook(payload)
    AF->>Git: getMRNotes() + getMRDiscussions()
    Git-->>AF: 返回 discussions 列表
    AF->>AF: 找到 /ai-fix noteId 所在的 discussionId
    AF->>Git: POST /discussions/{discussionId}/notes (收到命令)
    Note over Git: 评论出现在讨论串中 ✅
    AF->>AF: 执行修复...
    AF->>Git: PUT /notes/{noteId} (更新为"完成")
    Note over Git: 同一条评论更新，仍在讨论串中 ✅
```
...
```mermaid
graph TD
    A[MR update 触发评审] --> B{MongoDB 中有上次评审记录?}
    B -->|有| C[从 DB 取出上次 issues 列表]
    B -->|无| D[首次评审，无历史]
    C --> E[将历史 issues 附加到 AI prompt 中]
    D --> E
    E --> F[AI 评审当前 diff]
    F --> G[AI 返回 issues，每个标记 isNew]
    G --> H{isNew = true?}
    H -->|是| I[提交行内评论 ✅]
    H -->|否| J[跳过行内评论，在总体评论中列出 ⏭️]
    I --> K[更新总体评论]
    J --> K
```
...
- [warning] src/wheel/index.ts:5 — 无语义的常量 const a = 1
```
